...
def fallback_answer(prompt, language):
    ...
    if language == "English":
        return (
            ...
            "unauthorized access, attacks, or damage. It includes using strong passwords, "
            "updating software, avoiding suspicious links, and enabling two-factor authentication."
        )
    else:
        return (
            "خدمة الذكاء الاصطناعي الحية غير متوفرة حالياً، لذا هذه إجابة عامة:\n\n"
            "الأمن السيبراني هو حماية الحواسيب والشبكات والبيانات من الوصول غير المصرح به "
            "أو الهجمات أو التلف، ويتضمن استخدام كلمات مرور قوية وتحديث الأنظمة وتجنب الروابط "
            "المشبوهة وتفعيل المصادقة الثنائية."
        )
...
with st.sidebar:
    ...
    selected_page = st.radio("Choose a section:", nav_options, label_visibility="collapsed")

    st.markdown("---")
    st.markdown(f"**{get_text('progress')}**")
    st.metric(
        "Quiz Score" if st.session_state.language == "English" else "النتيجة",
    ...
    if st.session_state.chat_history:
        ...
        for msg in st.session_state.chat_history:
            if msg["role"] == "user":
                ...
                st.markdown(
                    f"**{'Assistant' if st.session_state.language == 'English' else 'المساعد'}:** {msg['content']}"
                )
            st.markdown("---")
...
# =========================
    ...
    if st.session_state.current_question >= len(current_questions):
        ...
        st.write(
            f"**{'Question' if st.session_state.language == 'English' else 'السؤال'} "
            f"{st.session_state.current_question + 1} "
            f"{'of' if st.session_state.language == 'English' else 'من'} "
            f"{len(current_questions)}**"
        )
        ...
        if st.button(get_text("quiz_submit"), use_container_width=True, key=f"submit_{st.session_state.current_question}"):
            if q["options"].index(selected) == q["correct"]:
                ...
                st.success(
                    "✅ "
                    + ("Correct!  Well done." if st.session_state.language == "English" else "صحيح! أحسنت.")
                )
            else:
                correct_answer = q["options"][q["correct"]]
                st.error(
                    f"❌ {'Incorrect.  The correct answer is:' if st.session_state.language == 'English' else 'غير صحيح. الإجابة الصحيحة هي:'} {correct_answer}"
                )
